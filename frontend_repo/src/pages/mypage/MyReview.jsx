...
const MyReviews = () => {
  ...
  return (
    <div className="flex flex-col items-center bg-[#f8f5e6] min-h-screen p-6 w-full max-w-md">
      <h3 className="text-lg font-bold mb-2">내가 작성한 리뷰 ({reviews.length})</h3>

      <div className="mt-4 space-y-4 w-full">
        {reviews.map((review) => (
          <div key={review.reviewId} className="border p-3 rounded-md bg-yellow-100">
            <p className="text-sm text-gray-700">
              상품 코드: {review.productCode} | 작성날짜: {formatDate(review.reviewDate)}
            </p>
      ...
      <button className="mt-4 bg-gray-500 text-white px-4 py-2 rounded-md" onClick={() => navigate("/mypage")}>
        마이페이지로 돌아가기
      </button>
    </div>
  );
